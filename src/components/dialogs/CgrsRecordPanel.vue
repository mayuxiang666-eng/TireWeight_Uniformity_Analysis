  ...
  <div class="cgrs-panel-root">
    ...
    <div class="cgrs-meta-bar">
      <div class="meta-left">
        ...
        <span v-if="eventTime" class="meta-badge date-badge" style="background: #eff6ff; color: #1d4ed8; border-color: #bfdbfe;">
          <el-icon><Clock /></el-icon>
          调参事件: <strong>{{ eventTime }}</strong>
        </span>
        <span v-else class="meta-badge date-badge">
      ...
      <div class="meta-right" style="display: flex; align-items: center; gap: 10px;">
        <button
          v-if="!hideRecommendBtn"
          type="button"
          class="switch-recommend-btn font-mono"
          @click="emit('open-recommend-dialog', { machine, article, stage: isCu ? 'ct' : 'gt' })"
        >
          💡 查看工艺底表四列推荐对比
        </button>
...
const props = defineProps({
  ...
  hideRecommendBtn: { type: Boolean, default: false },
  eventTime:        { type: String,  default: '' }
})

const emit = defineEmits(['open-recommend-dialog', 'close'])
...
const emptyDescriptionText = computed(() => {
  ...
  if (props.eventTime) {
    return `机台 [${props.machine}] 在调参时刻 [${props.eventTime}] 未查询到 CGRS 调参记录`
  }
...
async function fetchControlledAnalysis() {
  if (!props.machine || (!props.date && !props.eventTime)) return
  ...
  try {
    const res = await api.getCgrsControlledAnalysis({
      ...
      event_time: props.eventTime || undefined,
      ...
      same_day_only: props.eventTime ? false : sameDayOnly.value,
      min_samples: 1
    })
...
onMounted(() => {
  if (props.machine && (props.date || props.eventTime)) {
    fetchCgrsRecords()
  }
})

watch(
  [() => props.machine, () => props.date, () => props.article, () => props.indicator, () => props.eventTime],
  ([m, d, a, ind, et]) => {
    if (m && (d || et)) {
      fetchCgrsRecords()
    }
