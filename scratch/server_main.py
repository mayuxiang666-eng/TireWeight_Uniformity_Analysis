...
def handle_etl_reload():
    ok = reload_duckdb_data()
    if ok:
        return {"status": "success", "message": f"DuckDB 内存表已成功热重载最新 cleaned parquet 与 CGRS 数据"}
    return {"status": "error", "message": "未找到清洗后的 Parquet 或 CGRS 文件。"}
...
def get_periods(baseline_from=None, baseline_to=None, study_from=None, study_to=None):
    res = qry("SELECT MIN(TRY_CAST(tu_first_loc_timestamp AS TIMESTAMP))::DATE as min_d, MAX(TRY_CAST(tu_first_loc_timestamp AS TIMESTAMP))::DATE as max_d FROM clean_yield")[0]
    min_d_obj = res['min_d']
    max_d_obj = res['max_d']
        ...
        if best_tu_machine:
            ...
            for r in candidate_paths:
                ...
                if indicator == "weight":
                    ...
                    cpk_val = (global_usl - avg_v) / (3.0 * std_v) if std_v > 0.0 else 1.33
                    cpk_val = max(0.0, min(5.0, cpk_val))
                    q_score = cpk_val
...
#                 diag = "全局物料批次缺陷"
#                 sugg = "批次在该机台异常率均偏高，原材料存在自身缺陷，建议封存批次"
#             else:
#                 diag = "设备系统性工艺漂移"
