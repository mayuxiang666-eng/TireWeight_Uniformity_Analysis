...
DATA_PATH = "d:/Ava/untitled1/untitled1/src/yield_flat_table_joined_0714(2).parquet"
db_conn = duckdb.connect()
db_conn.execute(f"CREATE OR REPLACE TABLE clean_yield AS SELECT * FROM read_parquet('{DATA_PATH}')")

print("clean_yield table schema description:")
print(db_conn.execute("DESCRIBE clean_yield").df()[['column_name', 'column_type']])
...
spec_cpk AS (
    SELECT
        *,
        CASE 
            WHEN std_rfpp > 0 THEN (usl_rfpp - avg_rfpp) / (3.0 * std_rfpp)
            ELSE NULL 
        END AS cpk_rfpp,
        CASE 
            WHEN std_rfh1 > 0 THEN (usl_rfh1 - avg_rfh1) / (3.0 * std_rfh1)
            ELSE NULL 
        END AS cpk_rfh1
...
SELECT
    ...
    SUM(cpk_rfpp * sample_size) / SUM(CASE WHEN cpk_rfpp IS NOT NULL THEN sample_size ELSE 0 END) AS weighted_cpk_rfpp,
    SUM(cpk_rfh1 * sample_size) / SUM(CASE WHEN cpk_rfh1 IS NOT NULL THEN sample_size ELSE 0 END) AS weighted_cpk_rfh1
FROM spec_cpk
GROUP BY 1, 2
ORDER BY 1, 2
LIMIT 10
...
try:
    ...
    print("\nSQL executed successfully! Sample results:")
    print(res.to_string())
