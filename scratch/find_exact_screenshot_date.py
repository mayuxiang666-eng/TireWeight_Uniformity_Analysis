...
# Search over all dates
sql = """
    SELECT 
        ct_shiftdate::DATE as date,
        COUNT(*) as spec_n,
        AVG(TRY_CAST(rfppwc_first AS DOUBLE)) as spec_avg,
        STDDEV(TRY_CAST(rfppwc_first AS DOUBLE)) as spec_std
    FROM clean_yield 
...
for d, spec_n, spec_avg, spec_std in dates_list:
    if spec_n == 5:
        ...
        print(f"Date: {d}, spec_n: {spec_n}, all_n: {all_n}, spec_avg+3sigma: {avg_3s:.3f}")
